  ...
  programs.jujutsu = {
    ...
    settings = {
      ...
      revset-aliases = {
        ...
        "stack()" =
          "reachable(@, ~ ::trunk()) | ancestors(trunk()..@, 2) | trunk()";
      ...
      aliases = {
        ...
        ddl =
          [ "diff" "--git" "--config-toml=ui.pager='delta --line-numbers'" ];
        ...
        abandon-merged = [
          "abandon"
          "trunk().. & ..@ & empty() ~ @ ~ merges() ~ visible_heads()"
        ];
        simplify-merge = [
          "rebase"
          "-s"
          "heads(trunk()..@ & merges())"
          "-d"
          "heads(heads(trunk()..@ & merges())-)"
        ];
        sync =
          [ "rebase" "-s" "roots(trunk()..@)" "-d" "trunk()" "--skip-emptied" ];
        sync-all =
          [ "rebase" "-s" "roots(mutable())" "-d" "trunk()" "--skip-emptied" ];
